...
@CjService(name = "/alipay/pay_notify.html")
public class AlipayPayNotifyUrlWebView implements IGatewayAppSiteWayWebView {
    @CjServiceRef
    ...
    private void doPost(Frame frame, Circuit circuit) throws CircuitException {
        CJSystem.logging().info(getClass(), String.format("支付宝支付异步通知:%s", frame));
        ...
        String certPath2 = account.getCertPath2();
        try {
            boolean flag = AlipaySignature.certVerifyV1(params, certPath2, "utf-8", "RSA2");
            if (!flag) {
